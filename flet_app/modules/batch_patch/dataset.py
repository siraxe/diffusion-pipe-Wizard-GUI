...
class PipelineDataLoader:
    ...
    def state_dict(self):
        # Store number of examples seen instead of batches, so it's batch-size-agnostic
        # Get batch size from first bucket (assuming all buckets have same batch_size)
        if len(self.dataset.buckets) > 0:
            batch_size = self.dataset.buckets[0].batch_size
        else:
            batch_size = 1
        num_examples = self.num_batches_pulled * batch_size
        return {
            'epoch': self.epoch,
    ...
    def load_state_dict(self, state_dict):
        ...
        # Use num_examples if available (batch-size-agnostic), fall back to num_batches_pulled for backwards compatibility
        if 'num_examples' in state_dict:
            num_examples = state_dict['num_examples']
            old_batch_size = state_dict.get('batch_size', None)

            # Get current batch size from first bucket
            if len(self.dataset.buckets) > 0:
                current_batch_size = self.dataset.buckets[0].batch_size
            else:
                current_batch_size = 1

            # If batch size changed, reset dataloader to beginning of epoch instead of trying to resume
            # This is because changing batch size changes the dataset structure (number of items, bucket sizes, etc.)
            if old_batch_size is not None and old_batch_size != current_batch_size:
                # Batch size changed, reset to beginning (epoch 1) since dataset structure is now different
                self.epoch = 1  # Reset to epoch 1 when batch size changes
                self.num_batches_pulled = 0
                self.next_micro_batch = None
                self._create_dataloader()
                self.data = self._pull_batches_from_dataloader()
                self.recreate_dataloader = False
                if is_main_process():
                    print(f'Batch size changed from {old_batch_size} to {current_batch_size}. Resetting to epoch 1.')
                return

            # Batch size unchanged, resume from saved position
            num_batches_to_skip = num_examples // current_batch_size
            # Clamp to dataset size to avoid out of bounds
            num_batches_to_skip = min(num_batches_to_skip, len(self.dataset) - 1) if len(self.dataset) > 0 else 0
            # -1 for preloading
            if num_batches_to_skip > 0:
                num_batches_to_skip -= 1
        else:
            # Backwards compatibility with old checkpoints (no num_examples field)
            # These are old checkpoints that may have been saved with a different batch size
            num_batches_to_skip = state_dict['num_batches_pulled'] - 1

            # Safety check: if we would skip past the end of the dataset, reset to beginning
            # This handles cases where batch size changed between checkpoint save and resume
            if num_batches_to_skip >= len(self.dataset):
                if is_main_process():
                    print(f'Checkpoint skip position ({num_batches_to_skip}) >= dataset length ({len(self.dataset)}). '
                          f'This likely means batch size changed. Resetting to epoch 1.')
                self.epoch = 1
                self.num_batches_pulled = 0
                self.next_micro_batch = None
                self._create_dataloader()
        ...
        self.num_batches_pulled = num_batches_to_skip
        self.next_micro_batch = None  # Reset next_micro_batch to ensure fresh start
        self._create_dataloader(skip_first_n_batches=num_batches_to_skip)
        self.data = self._pull_batches_from_dataloader()
