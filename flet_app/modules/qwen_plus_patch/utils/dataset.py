...
class DirectoryDataset:
    def __init__(self, directory_config, dataset_config, model_name, framerate=None, skip_dataset_validation=False):
        ...
        self.reference_path = Path(self.directory_config['reference_path']) if 'reference_path' in self.directory_config else None
    ...
    def _get_ungrouped_metadata(self, regenerate_cache=False, trust_cache=False):
        ...
        if regenerate_cache or not metadata_cache_file_1.exists() or not trust_cache:
            ...
            reference_file_stems = {path.stem: path for path in self.reference_path.glob('*') if path.is_file()} if self.reference_path is not None else {}
                ...
                for image_spec in process_file(file):
                    ...
                    if self.reference_path:
                        if image_file.stem not in reference_file_stems:
                            raise RuntimeError(f'No reference file exists for image {image_file}')
                        reference_files.append(str(reference_file_stems[image_file.stem]))
...
def _cache_fn(datasets, queue, preprocess_media_file_fn, num_text_encoders, regenerate_cache, trust_cache, caching_batch_size):
    ...
    def latents_map_fn(example, rank):
        ...
        is_triple_image_dataset = ('reference_file' in example)
        first_size_bucket = example['size_bucket'][0]
        tensors_and_masks = []
        image_specs = []
        captions = []
        control_tensors_and_masks = []
        reference_tensors_and_masks = []
        for i, (image_spec, mask_path, size_bucket, caption) in enumerate(
            ...
            if is_triple_image_dataset:
                reference_file = example['reference_file'][i]
                reference_items = preprocess_media_file_fn((None, reference_file), None, size_bucket)
                assert len(reference_items) == 1
                assert len(items) == 1
                reference_tensors_and_masks.append(reference_items[0])
            else:
                reference_tensors_and_masks.append(None)
        ...
        for i in range(0, len(tensors_and_masks), caching_batch_size):
            ...
            r_tensor = torch.stack([t[0] for t in reference_tensors_and_masks[i:i+caching_batch_size]]) if is_triple_image_dataset else None
            parent_conn, child_conn = pipes.setdefault(rank, mp.Pipe(duplex=False))
            queue.put((0, tensor, c_tensor, r_tensor, child_conn))
    ...
    for text_encoder_idx in range(num_text_encoders):
        def text_embedding_map_fn(example, rank):
            ...
            reference_file = example['reference_file'] if 'reference_file' in example else None
            queue.put((text_encoder_idx+1, example['caption'], example['is_video'], control_file, reference_file, child_conn))
            result = parent_conn.recv()  # dict
...
class DatasetManager:
    def __init__(self, model, regenerate_cache=False, trust_cache=False, caching_batch_size=1):
        ...
        self.te_fn_requires_reference_file = [
            'reference_file' in signature(fn).parameters
            for fn in self.call_text_encoder_fns
        ]
    ...
    def _handle_task(self, task):
        ...
        if id == 0:
            tensor, control_tensor, reference_tensor, pipe = task[1:]
            if reference_tensor is not None:
                results = self.call_vae_fn(tensor, control_tensor, reference_tensor)
            elif control_tensor is not None:
                # edit dataset
                results = self.call_vae_fn(tensor, control_tensor)
            else:
                results = self.call_vae_fn(tensor)
        elif id > 0:
            caption, is_video, control_file, reference_file, pipe = task[1:]
