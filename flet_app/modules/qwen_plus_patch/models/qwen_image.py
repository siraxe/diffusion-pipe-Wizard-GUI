...
class QwenImagePipeline(BasePipeline):
    ...
    def __init__(self, config):
        ...
        tokenizer = transformers.Qwen2Tokenizer.from_pretrained(str(Path(self.model_config['diffusers_path']) / 'tokenizer'), local_files_only=True)
        processor = transformers.Qwen2VLProcessor.from_pretrained(str(Path(self.model_config['diffusers_path']) / 'processor'), local_files_only=True)
        ...
        text_encoder_config = transformers.Qwen2_5_VLConfig.from_pretrained(text_encoder_path, local_files_only=True)
        ...
        with open(str(Path(vae_path) / 'config.json')) as f:
    ...
    def load_diffusion_model(self):
        ...
        with open(str(Path(transformer_path) / 'config.json')) as f:
