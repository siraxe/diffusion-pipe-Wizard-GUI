...
def build_ltx2_toml_from_ui(training_tab_container, config_name: str = None) -> str:
    ...
    target_class_val = _get('target_class', 'cinematic scene')
    lines.append(f"target_class = {_quote(target_class_val)}")
    positive_val = _get('positive', 'a very sunny scene')
    lines.append(f"positive = {_quote(positive_val)}")
    negative_val = _get('negative', 'a very foggy scene')
    lines.append(f"negative = {_quote(negative_val)}")
    ...
    self_flow_args_val = _get('self_flow_args', 'student_block_ratio=0.3 teacher_block_ratio=0.7 lambda_self_flow=0.8 mask_ratio=0.1 teacher_momentum=0.9999 dual_timestep=true')
...
def update_ltx2_ui_from_toml(training_tab_container, toml_data: dict) -> None:
    ...
    try:
        ...
        _set_field_value('target_class', training_strategy.get('target_class', 'cinematic scene'))
        _set_field_value('positive', training_strategy.get('positive', 'a very sunny scene'))
        _set_field_value('negative', training_strategy.get('negative', 'a very foggy scene'))
        ...
        _set_field_value('self_flow_args', acceleration.get('self_flow_args', 'student_block_ratio=0.3 teacher_block_ratio=0.7 lambda_self_flow=0.8 mask_ratio=0.1 teacher_momentum=0.9999 dual_timestep=true'))
