...
def extract_config_from_controls(control):
    ...
    # Fields that should be extracted even when hidden (always include optimizer fields)
    always_include_fields = {
        'optimizer_type', 'optimizer_type_ltx2', 'grad_accum_steps', 'lr', 'betas', 'weight_decay', 'eps',
        'beta3', 'd0', 'd_coef', 'schedulefree_c'
    }

    # Field name mappings: internal label -> TOML key
    field_mappings = {
        'optimizer_type_ltx2': 'optimizer_type',
    ...
    def _extract(child):
        if hasattr(child, 'controls') and child.controls:
            ...
            # Always include optimizer-related fields, otherwise respect visibility
            ...
            # Always include optimizer-related fields, otherwise respect visibility
            label = getattr(child, 'label', None)
            # Map internal label to TOML key if mapping exists
            result_key = field_mappings.get(label, label)
            current_visible = getattr(child, 'visible', True)
            if label in always_include_fields:
                # For optimizer_type: prefer main variant over LTX2 variant (by label name)
                if result_key == 'optimizer_type':
                    # Main variant always wins, LTX2 variant only used if main not present
                    if label == 'optimizer_type' and current_visible:
                        if 'optimizer_type' not in result:
                            result[result_key] = child.value
                    elif label == 'optimizer_type_ltx2' and current_visible:
                        if 'optimizer_type' not in result:
                            result[result_key] = child.value
                else:
                    result[result_key] = child.value
            ...
            if getattr(child, 'visible', True):
                ...
                if child.label in ['8_bit_text_encoder']:
                    key = child.label
...
def _build_optimizer_section(lines, cfg, _get):
    ...
    # Check optimizer type for special handling
    optimizer_type = str(_get('optimizer_type', 'adamw_optimi')).lower()
    is_automagic = 'automagic' in optimizer_type

    # Write eps - skip for automagic (will be written in special format below)
    if not is_automagic:
        lines.append(f"eps = {_get('eps', 1e-8)}")

    # Prodigy-specific fields (only save if they have non-default values)
    if 'prodigy' in optimizer_type:
        # Only write beta3 if it's not None
        ...
        if schedulefree_c_val is not None and float(schedulefree_c_val) != 0.0:
            lines.append(f"schedulefree_c = {schedulefree_c_val}")

    # Automagic-specific fields (only save if they have non-default values)
    if is_automagic:
        ...
        do_paramiter_swapping_val = _get('do_paramiter_swapping', False)
        lines.append(f"do_paramiter_swapping = {'true' if _to_bool(do_paramiter_swapping_val) else 'false'}")
        lines.append(f"paramiter_swapping_factor = {_get('paramiter_swapping_factor', 0.1)}")
        # For automagic, use fixed eps format (TOML array)
        # Automagic requires eps as (min, max) tuple
...
def _build_adapter_section(lines, cfg, _get):
    ...
    # Only save init_from_existing if not empty
    init_from_existing = _get('init_from_existing', '')
    if init_from_existing and str(init_from_existing).strip():
        lines.append(f"init_from_existing = {_quote(str(init_from_existing).strip())}")
...
def build_toml_config_from_ui(training_tab_container) -> str:
    ...
    # Read init_from_existing from bottom bar field and resolve to absolute path
    init_from_existing_val = ""
    try:
        bb_field = getattr(training_tab_container, 'init_from_existing_field', None)
        if bb_field is not None and isinstance(bb_field, ft.TextField) and getattr(bb_field, 'value', None):
            init_from_existing_val = str(bb_field.value or '').strip()
    except Exception:
        pass

    # Resolve init_from_existing to absolute path for saving
    resolved_init_from_existing = ""
    if init_from_existing_val:
        init_from_existing_val = _normalize_slashes(init_from_existing_val)
        if _is_absolute_path(init_from_existing_val):
            resolved_init_from_existing = os.path.normpath(init_from_existing_val)
        else:
            resolved_init_from_existing = os.path.normpath(os.path.join(str(project_root), *init_from_existing_val.split('/')))
        resolved_init_from_existing = _normalize_slashes(resolved_init_from_existing)

    # Add resolved init_from_existing to cfg dict for adapter section
    ...
    # Add [data] section with dataset_list for UI state (stores selected dataset names)
    dataset_names = []
    ...
    if isinstance(model, dict):
        ...
        if normalized_type:
            ...
            if 'timestep_sample_method' in model:
                ...
                # For non-LTX2 models, map invalid timestep methods to valid ones
                # The diffusion-pipe trainer only supports 'logit_normal' and 'uniform'
                if not is_ltx2 and ts_method == 'shifted_logit_normal':
...
def _populate_optimizer_section(toml_data, label_vals):
    ...
    if isinstance(opt, dict):
        ...
        for k in ('min_lr', 'max_lr', 'lr_bump', 'clip_threshold', 'do_paramiter_swapping', 'paramiter_swapping_factor'):
            if k in opt:
                label_vals[k] = opt.get(k)
    ...
    if isinstance(ad, dict):
        ...
        if 'lora_targets' in ad:
            ...
            if isinstance(lora_targets, dict):
                # No LTX2 module checkboxes to populate anymore
                pass
...
def _populate_training_strategy_section(toml_data, label_vals, _to_bool_func):
    ...
    if isinstance(training_strategy, dict):
        ...
        if 'ltx_mode' in training_strategy:
            label_vals['ltx_mode'] = training_strategy.get('ltx_mode')
        if 'separate_audio_buckets' in training_strategy:
            label_vals['separate_audio_buckets'] = _to_bool(training_strategy.get('separate_audio_buckets', False))
        if 'slider' in training_strategy:
            label_vals['slider'] = _to_bool(training_strategy.get('slider', False))
        if 'use_mask' in training_strategy:
            label_vals['use_mask'] = _to_bool(training_strategy.get('use_mask', False))
...
def _populate_optimization_section(toml_data, label_vals):
    ...
    if isinstance(optimization, dict):
        ...
        if 'enable_gradient_checkpointing' in optimization:
            label_vals['gradient_checkpointing'] = optimization.get('enable_gradient_checkpointing')
...
def _populate_acceleration_section(toml_data, label_vals, _to_bool_func):
    ...
    if isinstance(acceleration, dict):
        ...
        if 'fp8_base' in acceleration:
            label_vals['fp8_base'] = _to_bool(acceleration.get('fp8_base', True))
        if 'fp8_scaled' in acceleration:
            label_vals['fp8_scaled'] = _to_bool(acceleration.get('fp8_scaled', True))
...
def _handle_dataset_list_selection(toml_data, training_tab_container, page):
    """Handle dataset_list to populate multiple dataset blocks."""
    try:
        # Check data.dataset_list first (for LTX2 configs), then top-level dataset_list
        dataset_list = None
        data_section = toml_data.get('data', {})
        if isinstance(data_section, dict):
        ...
        # Get the config page content which contains dataset_1_block, dataset_2_block, dataset_3_block
        config_page_content = getattr(training_tab_container, 'config_page_content', None)
        if not config_page_content:
            return

        # First, clear all 3 dataset blocks to reset any previous selections
        for i in range(1, 4):
            ds_block = getattr(config_page_content, f'dataset_{i}_block', None)
            if ds_block and hasattr(ds_block, 'set_selected_dataset'):
                try:
        ...
        # Then populate the dataset blocks with the datasets from dataset_list
        for i, dataset_name in enumerate(dataset_list):
            if i >= 3:  # Max 3 dataset blocks
                break
            ds_block = getattr(config_page_content, f'dataset_{i+1}_block', None)
            if ds_block and hasattr(ds_block, 'set_selected_dataset'):
                try:
...
def update_ui_from_toml(training_tab_container, toml_data: dict):
    ...
    # Post-process init_from_existing for UI: strip project root to show relative workspace path
    if 'init_from_existing' in label_vals and isinstance(label_vals['init_from_existing'], str):
        label_vals['init_from_existing'] = _collapse_path_to_relative(label_vals['init_from_existing'])
    ...
    # Bottom bar init_from_existing
    try:
        if 'init_from_existing' in label_vals:
            bb_field = getattr(training_tab_container, 'init_from_existing_field', None)
            if bb_field is not None and isinstance(bb_field, ft.TextField):
                bb_field.value = str(label_vals['init_from_existing']) if label_vals['init_from_existing'] is not None else ''
                if bb_field.page:
                    bb_field.update()
    except Exception:
        pass
    ...
    # Handle dataset_list from [data] section for multiple dataset blocks
    _handle_dataset_list_selection(toml_data, training_tab_container, page)
    ...
    try:
        ...
        # Handle optimizer type change to show/hide prodigy row
        if optimizer_type_dropdown_ref and optimizer_type_dropdown_ref.current:
            optimizer_type = label_vals.get('optimizer_type')
            if optimizer_type:
                # Trigger the change handler to show/hide prodigy row
                # Pass from_toml_load=True so values aren't reset to defaults
                from flet_app.ui.pages.training_config import on_optimizer_type_change
                class _E:
                    pass
                e = _E()
                setattr(e, 'control', optimizer_type_dropdown_ref.current)
                setattr(e, 'data', str(optimizer_type))
                setattr(e, 'page', page)
