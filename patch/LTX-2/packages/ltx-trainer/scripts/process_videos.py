...
class MediaDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            enable_ar_bucket: Enable smart AR routing to closest bucket
        """
        super().__init__()
        ...
        # Validate and adjust resolution buckets to be divisible by VAE_SPATIAL_FACTOR (32)
        validated_buckets = []
        for frames, height, width in resolution_buckets:
            adj_height = (height // VAE_SPATIAL_FACTOR) * VAE_SPATIAL_FACTOR
            adj_width = (width // VAE_SPATIAL_FACTOR) * VAE_SPATIAL_FACTOR
            if adj_height != height or adj_width != width:
                logger.warning(
                    f"Bucket {width}x{height} adjusted to {adj_width}x{adj_height} for VAE compatibility"
                )
            validated_buckets.append((frames, adj_height, adj_width))
        self.resolution_buckets = validated_buckets
    ...
    def _get_resolution_bucket_for_item(self, media_tensor: torch.Tensor) -> tuple[int, int, int]:
        ...
        # Smart AR routing: when enable_ar_bucket=True and 2+ buckets, pick based on closest aspect ratio
        if self.enable_ar_bucket and len(relevant_buckets) >= 2:
            media_ar = width / height

            def ar_distance(bucket: tuple[int, int, int]) -> float:
                _, bucket_height, bucket_width = bucket
                bucket_ar = bucket_width / bucket_height
                return abs(media_ar - bucket_ar)

            nearest_bucket = min(relevant_buckets, key=ar_distance)
            logger.debug(
                f"AR routing: media {width}x{height} (AR={media_ar:.2f}) -> "
                f"bucket {nearest_bucket[2]}x{nearest_bucket[1]} (AR={nearest_bucket[2]/nearest_bucket[1]:.2f})"
            )
...
def compute_latents(  # noqa: PLR0913, PLR0915
    ...
    # Count videos per resolution bucket
    from ltx_trainer.video_utils import read_video, get_video_frame_count
    from ltx_trainer.utils import open_image_as_srgb
    ...
    for idx in range(len(dataset)):
        ...
        if video_path.suffix.lower() in [".png", ".jpg", ".jpeg"]:
            ...
            image = open_image_as_srgb(video_path)
            image_tensor = to_tensor(image).unsqueeze(0)  # [1, C, H, W]
            bucket = dataset._get_resolution_bucket_for_item(image_tensor)
        else:
            # For videos, get actual frame count and dimensions
            frame_count = get_video_frame_count(video_path)
            video, _ = read_video(video_path, max_frames=1)
            # Create a tensor with actual frame count for bucket selection
            # [frames, C, H, W] - using max_target_frames to ensure bucket compatibility
            max_frames = dataset.max_target_frames
            dummy_video = torch.zeros((max_frames, video.shape[1], video.shape[2], video.shape[3]))
            dummy_video[0] = video[0]  # Set first frame
            bucket = dataset._get_resolution_bucket_for_item(dummy_video)
        bucket_key = f"{bucket[2]}x{bucket[1]}x{bucket[0]}"
...
def main(  # noqa: PLR0913
    ...
    enable_ar_bucket: bool = typer.Option(
        default=False,
        help="Enable smart AR routing to closest aspect ratio bucket",
    ),
) -> None:
