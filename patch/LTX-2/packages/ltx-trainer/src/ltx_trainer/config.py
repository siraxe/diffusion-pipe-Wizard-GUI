...
class ModelConfig(ConfigBaseModel):
    ...
    load_checkpoint_extra: str | Path | None = Field(
        default=None,
        description="Path to an additional LoRA checkpoint to blend with the main checkpoint. "
        "Blending formula: main + (lce_opacity × extra). "
        "If a directory is provided, the latest checkpoint will be used.",
    )

    lce_opacity: float = Field(
        default=1.0,
        description="Blend opacity for load_checkpoint_extra. Range 0.0-1.0. "
        "The extra checkpoint is multiplied by this value before blending with the main checkpoint.",
        ge=0.0,
        le=1.0,
    )
